...
### Requirement: Mock transport 必須與 protobuf-ts 的 `RpcInterceptor` 管線相容

由 generated client 的 `stackIntercept()` 呼叫時，`MockRpcTransport` SHALL 回傳符合 `UnaryCall` / `ServerStreamingCall` 完整契約的 call object，使得掛在 `RpcOptions.interceptors` 的任何 `RpcInterceptor`（例如將事件送到 gRPC DevTools Extension 的 `docs/devtool.ts` `devtoolsInterceptor`）看到的事件序列，與使用真實 `GrpcWebFetchTransport` 時一致。函式庫本身 SHALL NOT 相依或觸發任何 DevTools 特定邏輯；DevTools 顯示完全由使用者在 `RpcOptions.interceptors` 上提供的 interceptor 負責。

- Unary：call object MUST 在建立時就暴露 `method`、`requestHeaders`、`request`，且 `headers`、`response`、`status`、`trailers` 必須為 `Promise`；`call.then(resolved, rejected)` MUST 在 resolver 完成後呼叫 `resolved(finishedUnaryCall)`，或在錯誤時呼叫 `rejected(error)`。
- Server streaming：call object MUST 暴露 `responses.onMessage` / `onComplete` / `onError` 訂閱點；interceptor 附上的 listener MUST 依 resolver emission 順序收到訊息，並在結束時收到正好一次 `onComplete`（成功）或 `onError`（錯誤）。
- Abort：當 `options.abort` 觸發時，call MUST 停止送出後續訊息，並以 `RpcError('CANCELLED', …)` 終止對應 promise / stream。

#### Scenario: Interceptor 看到 unary 成功事件序列
- **WHEN** 開發者在 `RpcOptions.interceptors` 加入 `docs/devtool.ts` 的 `devtoolsInterceptor`，並以 mock transport 呼叫 unary method，resolver 回傳 `{message: 'hi'}`
- **THEN** interceptor `interceptUnary` MUST 被呼叫恰好一次，並拿到 `call.method.name`、`call.method.service.typeName`、`call.request`、`call.requestHeaders`
- **AND** `call.then(...)` 的 resolved handler MUST 被呼叫恰好一次，且 `finishedUnaryCall.response` 等同 resolver 回傳值、`finishedUnaryCall.headers` 等同 resolver 提供的 headers（若有）

#### Scenario: Interceptor 看到 unary 錯誤事件序列
- **WHEN** resolver 丟出 `new RpcError('not found', 'NOT_FOUND')`，且 interceptor 同時掛載
- **THEN** `call.then` 的 rejected handler MUST 被呼叫恰好一次，且收到的 `error` 具有 `name`、`code === 'NOT_FOUND'`、`message === 'not found'`、`meta`
- **AND** `call.then` 的 resolved handler MUST NOT 被呼叫

#### Scenario: Interceptor 看到 server streaming 事件序列
- **WHEN** 以 mock transport 呼叫 server-streaming method，resolver 以 `[a, b]` 形式回傳兩筆訊息
- **THEN** interceptor 加到 `call.responses.onMessage` 的 listener MUST 依序收到 `a`、`b`，然後 `onComplete` listener MUST 被呼叫恰好一次
- **AND** `onError` listener MUST NOT 被呼叫

#### Scenario: Interceptor 看到 server streaming 的中途錯誤
- **WHEN** resolver 先送出一筆訊息，接著丟出 `RpcError('boom', 'INTERNAL')`
- **THEN** `onMessage` MUST 被呼叫一次，`onError` MUST 在其後被呼叫恰好一次且帶有對應 `RpcError`
- **AND** `onComplete` MUST NOT 被呼叫

#### Scenario: Interceptor 看到 abort 事件序列
- **WHEN** 呼叫端以 `AbortController` 在 resolver emit 過程中觸發 abort
- **THEN** 已送出的訊息 MUST 透過 `onMessage` 如實交給 interceptor
- **AND** 後續 MUST 只觸發一次 `onError`（`RpcError` code 為 `CANCELLED`），不得再觸發 `onComplete` 或 `onMessage`
