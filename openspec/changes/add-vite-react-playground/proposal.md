...
- 在 playground mock 層新增一層 MSW-like helper，讓範例可用宣告式 handler 陣列註冊 unary mock，並示範目前 session 內的 stateful mock。
- 擴充 playground proto 與 UI，展示 mutation-like unary 呼叫更新 session state 後，後續 query-like unary 呼叫會讀到更新後資料。
...
- **新增目錄**：`playground/`，包含 `package.json`、Vite/React 設定、`.proto`、generated code、mock helper、stateful mock handlers 與 UI source。
...
- **公開 API 影響**：helper 僅存在於 playground，暫不加入 `src/` 或 package `exports`，避免在 DX 尚未穩定前擴大核心 API surface。
